...
export function PublicLegalLayout({ eyebrow, title, description, lastUpdated, children }: { eyebrow: string; title: string; description: string; lastUpdated: string; children: ReactNode }) {
  return <div className="min-h-screen bg-[#FBFAF6] text-[#303A43]">
    <header className="border-b border-black/5 bg-white"><div className="mx-auto flex h-20 max-w-6xl items-center justify-between px-5 sm:px-8"><Link to="/" aria-label="ZestIQ home" className="rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F5D62E]"><ZestIQBrand /></Link><div className="flex items-center gap-2"><Link to="/legal" className="rounded-xl border border-black/10 bg-white px-4 py-2.5 text-sm font-black">Legal centre</Link><Link to="/book-demo" className="hidden rounded-xl bg-[#303A43] px-4 py-2.5 text-sm font-black text-white sm:block">Book a demo</Link></div></div></header>
    <main>
      <section className="border-b border-black/5 bg-[#303A43] text-white"><div className="mx-auto max-w-6xl px-5 py-14 sm:px-8 sm:py-20"><Link to="/" className="inline-flex items-center gap-2 text-sm font-bold text-white/60 hover:text-white"><ArrowLeft className="h-4 w-4" />Back to ZestIQ</Link><p className="mt-10 text-xs font-black uppercase tracking-[0.2em] text-[#F5D62E]">{eyebrow}</p><h1 className="mt-4 max-w-4xl text-4xl font-black tracking-[-0.04em] sm:text-6xl">{title}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-white/65">{description}</p><p className="mt-6 text-sm font-bold text-white/40">Effective and last updated: {lastUpdated}</p></div></section>
