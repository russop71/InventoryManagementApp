...
const pages = {
  ...
  pricing: {
    ...
    description: 'ZestIQ Basic costs CAD $249.99 per month for the first location. Additional locations are CAD $199.99 each. Scheduling is $49.99 plus $24.99 per additional location.',
    eyebrow: 'Clear Canadian pricing',
    heading: 'Start with ZestIQ Basic for CAD $249.99 per month.',
    intro: 'Basic is CAD $249.99 per month for the first restaurant location and includes the core inventory, costing, purchasing and AI platform. Each additional location is CAD $199.99 per month. Scheduling is an optional add-on.',
    sections: [
...
function PublicMarketingPage({ pageKey }: { pageKey: PublicPageKey }) {
  ...
  return <div className="grey-green-preview min-h-screen bg-[#FBFAF6] text-[#303A43]">
    <header className="border-b border-black/5 bg-white"><div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8"><Link to="/" aria-label="ZestIQ home" className="rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F58220]"><ZestIQBrand compact /></Link><div className="flex items-center gap-2"><Link to="/login" className="rounded-xl border border-black/10 px-4 py-2.5 text-sm font-black">Log in</Link><Link to="/book-demo" className="rounded-xl bg-[#303A43] px-4 py-2.5 text-sm font-black text-white">Book a demo</Link></div></div></header>
    <main>
      <section className="overflow-hidden bg-[#303A43] text-white"><div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[1fr_.82fr] lg:items-center lg:py-28"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#F58220]">{page.eyebrow}</p><h1 className="mt-5 max-w-4xl text-5xl font-black leading-[.95] tracking-[-.045em] sm:text-6xl">{page.heading}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-white/65">{page.intro}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link to="/book-demo" className="inline-flex h-14 items-center justify-center gap-2 rounded-xl bg-[#F58220] px-7 font-black text-[#303A43]">Book a tailored demo<ArrowRight className="h-4 w-4" /></Link><button type="button" onClick={openDemoAccount} disabled={demoLoading} className="inline-flex h-14 items-center justify-center rounded-xl border border-white/15 px-7 font-black transition hover:border-[#F58220]/60 hover:bg-white/5 disabled:cursor-wait disabled:opacity-60">{demoLoading ? 'Opening demo…' : 'Explore the product'}</button></div></div><HeroVisual pageKey={pageKey} /></div></section>
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8"><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{page.sections.map(([Icon, title, text]) => <article key={title} className="rounded-3xl border border-black/10 bg-white p-7 shadow-sm"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#FFDCAD]"><Icon className="h-6 w-6" /></div><h2 className="mt-5 text-2xl font-black">{title}</h2><p className="mt-3 leading-7 text-black/55">{text}</p></article>)}</div></section>
      <section className="bg-[#F58220]"><div className="mx-auto flex max-w-7xl flex-col gap-6 px-5 py-14 sm:px-8 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[.2em] text-black/45">Next step</p><h2 className="mt-2 text-3xl font-black">See how this fits your restaurant.</h2></div><div className="flex flex-col gap-3 sm:flex-row"><a href="mailto:demo@zestiq.ca" className="inline-flex h-12 items-center justify-center rounded-xl border-2 border-[#303A43] px-6 font-black">Email ZestIQ</a><Link to="/book-demo" className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[#303A43] px-6 font-black text-white">Book a demo<ArrowRight className="h-4 w-4" /></Link></div></div></section>
    </main>
...
function HeroVisual({ pageKey }: { pageKey: PublicPageKey }) {
  return <div className="relative hidden place-items-center lg:grid">
    <div className="absolute h-72 w-72 rounded-full bg-[#F58220]/15 blur-3xl" />
