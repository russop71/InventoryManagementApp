...
function SeoPage({ pageKey }: { pageKey: PageKey }) {
  ...
  return <div className="grey-green-preview min-h-screen bg-[#FBFAF6] text-[#303A43]">
    ...
    <main>
      <section className="bg-[#303A43] text-white"><div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[0.92fr_1.08fr] lg:items-center lg:py-28"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-[#F58220]">{page.eyebrow}</p><h1 className="mt-5 text-5xl font-black leading-[0.96] tracking-[-0.045em] sm:text-6xl">{page.heading}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-white/65">{page.intro}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link to="/book-demo" className="inline-flex h-14 items-center justify-center gap-2 rounded-xl bg-[#F58220] px-7 font-black text-[#303A43]">See ZestIQ in action<ArrowRight className="h-4 w-4" /></Link><Link to="/" className="inline-flex h-14 items-center justify-center rounded-xl border border-white/15 px-7 font-bold">Explore the platform</Link></div></div><div className="rounded-[30px] border border-white/10 bg-white/5 p-6"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-[#F58220] text-[#303A43]"><Icon className="h-7 w-7" /></div><p className="mt-6 text-sm font-black uppercase tracking-[0.16em] text-white/35">Connected restaurant control</p><div className="mt-4 space-y-3">{page.points.map(point => <div key={point} className="flex items-start gap-3 rounded-2xl bg-white/8 p-4"><Check className="mt-0.5 h-4 w-4 shrink-0 text-[#F58220]" /><p className="text-sm font-bold leading-6">{point}</p></div>)}</div></div></div></section>

      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8"><p className="text-xs font-black uppercase tracking-[0.2em] text-[#F58220]">A practical workflow</p><h2 className="mt-3 max-w-3xl text-4xl font-black tracking-[-0.035em] sm:text-5xl">From restaurant data to a decision your team can use.</h2><div className="mt-10 grid gap-4 lg:grid-cols-3">{page.workflow.map(([title, description], index) => <article key={title} className="rounded-3xl border border-black/10 bg-white p-7"><p className="text-sm font-black text-[#F58220]">0{index + 1}</p><h3 className="mt-7 text-2xl font-black">{title}</h3><p className="mt-3 leading-7 text-black/55">{description}</p></article>)}</div></section>

      {pageKey === 'inventory' && <section className="border-y border-black/5 bg-white"><div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:items-center"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-[#F58220]">Real ZestIQ inventory workflow</p><h2 className="mt-3 text-4xl font-black tracking-[-0.035em] sm:text-5xl">Built for restaurant units, storage areas and weekly decisions.</h2><p className="mt-5 max-w-2xl text-lg leading-8 text-black/55">Restaurant inventory is not generic warehouse stock. ZestIQ keeps cases, kilograms, bottles, pours and portions connected with supplier prices, recipes, waste and ordering.</p><div className="mt-8 grid gap-4 sm:grid-cols-2"><SmallFeature icon={PackageSearch} title="Count by storage area" text="Walk the walk-in, freezer, dry storage and bar in the order your team works." /><SmallFeature icon={ReceiptText} title="Use current invoice cost" text="Bring reviewed supplier pricing into inventory value and affected recipe margins." /><SmallFeature icon={TrendingDown} title="Investigate variance" text="Compare expected and actual usage, then focus on the highest-dollar exceptions." /><SmallFeature icon={ShoppingCart} title="Review suggested orders" text="Combine on-hand stock, pars, incoming purchases, demand and supplier packs." /></div></div><figure className="overflow-hidden rounded-[30px] border border-black/10 bg-[#303A43] p-3 shadow-2xl"><img src="/product-inventory.png" alt="ZestIQ restaurant inventory management dashboard showing item counts, units and stock information" width="1200" height="800" loading="lazy" className="h-auto w-full rounded-[22px]" /><figcaption className="px-3 pb-2 pt-4 text-sm font-bold text-white/55">ZestIQ inventory management—real product screen.</figcaption></figure></div></section>}

      {(pageKey === 'inventory' || pageKey === 'foodCost') && <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8"><div className="rounded-[32px] bg-[#303A43] p-7 text-white md:p-10"><p className="text-xs font-black uppercase tracking-[0.2em] text-[#F58220]">Restaurant operator resources</p><h2 className="mt-3 max-w-3xl text-3xl font-black tracking-[-0.03em] sm:text-4xl">Use the numbers to choose—and run—the right system.</h2><p className="mt-4 max-w-3xl leading-7 text-white/60">Compare restaurant inventory software with a practical scorecard, tighten the weekly count process and check recipe economics with current food-cost tools.</p><div className="mt-8 grid gap-3 md:grid-cols-2 lg:grid-cols-4"><RelatedResource to="/best-restaurant-inventory-management-software-canada" title="Canadian buyer’s guide" text="Compare counting, invoices, food cost, ordering and controls." /><RelatedResource to="/restaurant-inventory-management-guide" title="Inventory management guide" text="Build a repeatable count, variance and ordering rhythm." /><RelatedResource to="/restaurant-food-cost-calculator" title="Food-cost calculator" text="Check food-cost percentage, gross profit and target price." /><RelatedResource to="/pricing" title="ZestIQ pricing" text="Review transparent Canadian pricing and location costs." /></div></div></section>}

      <section className="bg-[#F58220]"><div className="mx-auto grid max-w-7xl gap-8 px-5 py-20 sm:px-8 lg:grid-cols-[0.7fr_1.3fr] lg:items-start"><div><Sparkles className="h-8 w-8" /><p className="mt-4 text-xs font-black uppercase tracking-[0.2em] text-black/45">Frequently asked</p></div><div className="divide-y divide-black/15">{faqs.map(([question, answer]) => <article key={question} className="py-7 first:pt-0 last:pb-0"><h2 className="text-2xl font-black tracking-tight sm:text-3xl">{question}</h2><p className="mt-4 max-w-3xl text-lg font-semibold leading-8 text-black/60">{answer}</p></article>)}</div></div></section>
...
function SmallFeature({ icon: Icon, title, text }: { icon: typeof BarChart3; title: string; text: string }) { return <div><Icon className="h-5 w-5 text-[#F58220]" /><h3 className="mt-3 font-black">{title}</h3><p className="mt-1 text-sm leading-6 text-black/50">{text}</p></div>; }
function RelatedResource({ to, title, text }: { to: string; title: string; text: string }) { return <Link to={to} className="group rounded-2xl border border-white/10 bg-white/5 p-5 transition hover:-translate-y-0.5 hover:bg-white/10"><h3 className="font-black text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-white/55">{text}</p><span className="mt-4 inline-flex items-center gap-2 text-sm font-black text-[#F58220]">Read more<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></span></Link>; }
