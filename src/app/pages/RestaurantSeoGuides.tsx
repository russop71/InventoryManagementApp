...
function Checklist({ items }: { items: string[] }) { return <ul className="grid gap-3">{items.map(item => <li key={item} className="flex gap-3"><Check className="mt-1.5 h-4 w-4 shrink-0 text-[#F58220]" /><span>{item}</span></li>)}</ul>; }
function Faq({ items }: { items: Array<[string, string]> }) { return <section className="rounded-3xl bg-[#F58220] p-6 sm:p-8"><h2 className="text-3xl font-black">Frequently asked questions</h2><div className="mt-6 grid gap-5">{items.map(([q, a]) => <div key={q}><h3 className="text-lg font-black">{q}</h3><p className="mt-2 leading-7 text-black/60">{a}</p></div>)}</div></section>; }
...
function Result({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl bg-white/8 p-5"><p className="text-xs font-bold uppercase tracking-[.12em] text-white/45">{label}</p><p className="mt-2 text-3xl font-black text-[#F58220]">{value}</p></div>; }
