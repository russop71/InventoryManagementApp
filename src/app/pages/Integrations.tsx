...
export function Integrations() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
        ...
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {POS_PROVIDERS.map(item => {
              ...
              return <button key={item.id} type="button" disabled={isDemoAccount} onClick={() => openProviderSettings(item.id)} aria-label={`Open ${item.name} integration settings`} className={`group rounded-2xl border p-4 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F58220] focus-visible:ring-offset-2 ${active ? 'border-[#F58220] bg-[#FEFCE8] ring-2 ring-[#F58220]/20' : 'border-slate-200 bg-white hover:-translate-y-0.5 hover:border-[#D6B900] hover:shadow-md'} ${isDemoAccount ? 'cursor-not-allowed opacity-70' : ''}`}>
                <div className="flex items-start justify-between gap-3"><span className="flex h-10 min-w-10 items-center justify-center rounded-xl px-2 text-xs font-black text-white" style={{ backgroundColor: item.colour }}>{item.mark}</span>{active && <CheckCircle className="h-5 w-5 text-[#F58220]" />}</div>
                <p className="mt-3 font-black text-slate-900">{item.name}</p><p className="mt-1 min-h-10 text-xs leading-5 text-slate-500">{item.description}</p><div className="mt-3 flex items-center justify-between gap-2"><span className="inline-flex rounded-full bg-slate-100 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-slate-500">{item.connection}</span><span className="inline-flex items-center gap-1 text-[10px] font-black text-[#F58220]">{isDemoAccount ? 'Preview' : 'Set up'}<ChevronRight className="h-3.5 w-3.5 transition group-hover:translate-x-0.5" /></span></div>
              </button>;
            })}
          </div>
        </CardContent>
      ...
      {isDemoAccount && <Card className="border-lime-200 bg-lime-50"><CardContent className="py-4"><p className="font-semibold text-[#F58220]">Integration setup is read-only in the public demo.</p><p className="mt-1 text-sm text-[#F58220]">The Toast connection and sample sales remain available to explore. Connectors, imports and disconnection are enabled in a private workspace.</p></CardContent></Card>}

      <div ref={setupRef} id="integration-settings" className="scroll-mt-5 rounded-[28px] border border-[#F58220]/60 bg-[#FFFDF1] p-3 shadow-sm sm:p-5">
        <div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div className="flex items-center gap-3"><span className="flex h-11 min-w-11 items-center justify-center rounded-xl px-2 text-xs font-black text-white" style={{ backgroundColor: selected.colour }}>{selected.mark}</span><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#F58220]">Integration settings</p><h2 className="text-xl font-black text-slate-950">Set up {selected.name}</h2></div></div><Badge className={selectedIsConnected ? 'bg-emerald-100 text-[#F58220]' : 'bg-white text-slate-700'}>{selectedIsConnected ? 'Data active' : 'Not connected'}</Badge></div>
        <Tabs value={setupMode} onValueChange={value => setSetupMode(value as 'import' | 'direct')} className="gap-4">
          <TabsList className="h-11 w-full bg-[#303A43] p-1 text-white sm:w-auto"><TabsTrigger value="import" className="px-5 text-white data-[state=active]:bg-[#F58220] data-[state=active]:font-black data-[state=active]:text-[#303A43]"><FileSpreadsheet className="h-4 w-4" />Import a file</TabsTrigger><TabsTrigger value="direct" className="px-5 text-white data-[state=active]:bg-[#F58220] data-[state=active]:font-black data-[state=active]:text-[#303A43]"><PlugZap className="h-4 w-4" />Direct connection</TabsTrigger></TabsList>
          <TabsContent value="import">
        <Card className="overflow-hidden">
          <CardHeader className="border-b border-slate-100"><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2 text-lg"><FileSpreadsheet className="h-5 w-5 text-[#F58220]" />Import sales now</CardTitle><CardDescription className="mt-1">Upload a {selected.name} CSV or JSON export. ZestIQ recognizes common item, quantity, revenue, category, date and cover headers.</CardDescription></div><Badge className="bg-emerald-100 text-[#F58220]">Ready</Badge></div></CardHeader>
          <CardContent className="space-y-4 pt-5">
            <label htmlFor="pos-file" className="flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 px-5 py-8 text-center transition hover:border-[#F58220] hover:bg-[#FEFCE8]">
        ...
        <Card className="overflow-hidden">
          <CardHeader className="border-b border-slate-100"><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2 text-lg"><PlugZap className="h-5 w-5 text-[#F58220]" />Direct sync</CardTitle><CardDescription className="mt-1">Automatic daily sales and menu updates.</CardDescription></div><Badge variant="outline">Activation required</Badge></div></CardHeader>
          <CardContent className="space-y-4 pt-5">
            ...
            {isDemoAccount
              ...
              : selected.id === 'clover'
                ? <Button type="button" onClick={handleCloverConnection} disabled={isConnecting} className="w-full bg-[#F58220] font-black text-[#303A43] hover:bg-[#E5B60D]">{isConnecting ? 'Connecting…' : selectedIsConnected ? 'Sync Clover now' : 'Connect Clover securely'}</Button>
                : <a href={requestUrl} className="flex h-10 w-full items-center justify-center rounded-md bg-[#F58220] px-4 text-sm font-black text-[#303A43] hover:bg-[#E5B60D]">Request direct activation</a>}
      ...
      <Card>
        <CardHeader><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><CardTitle className="text-lg">Sales connection status</CardTitle><CardDescription>{isConnected ? `${currentProvider.name} data is available to ZestIQ.` : 'No real sales data has been connected for this location yet.'}</CardDescription></div><Badge className={isConnected ? 'bg-emerald-100 text-[#F58220]' : 'bg-slate-100 text-slate-700'}>{isConnected ? <><CheckCircle className="mr-1 h-3 w-3" />Data active</> : <><XCircle className="mr-1 h-3 w-3" />Not connected</>}</Badge></div></CardHeader>
        <CardContent>
...
function Feature({ icon: Icon, text }: { icon: typeof ShieldCheck; text: string }) { return <div className="flex items-center gap-2"><Icon className="h-4 w-4 text-[#F58220]" /><span>{text}</span></div>; }
