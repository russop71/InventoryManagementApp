...
function EmployeeRow({ employee, onRemove }: { employee: LaborEmployee; onRemove: () => void }) {
  ...
  return <div className="grid gap-3 p-4 sm:grid-cols-[minmax(0,1.5fr)_minmax(110px,0.65fr)_minmax(100px,0.55fr)_auto] sm:items-center"><div className="flex min-w-0 items-center gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#FEF3C7] font-black text-[#0B1220]">{employee.name.split(' ').map(part => part[0]).slice(0, 2).join('')}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate font-black text-slate-900">{employee.name}</p><span className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase ${employee.inviteStatus === 'pending' ? 'bg-amber-50 text-amber-700' : employee.inviteStatus === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{employee.inviteStatus.replace('-', ' ')}</span></div><p className="mt-1 truncate text-xs font-semibold text-slate-500">{employee.role} · {employee.department}</p><p className="mt-1 truncate text-xs text-slate-400">{employee.email || employee.phone || 'No contact details'}</p></div></div><div className="rounded-xl bg-slate-50 px-3 py-2"><p className="text-[9px] font-black uppercase tracking-wider text-slate-400">Clock-in #</p><p className="mt-1 font-black tabular-nums text-slate-900">{employee.clockInNumber || '—'}</p></div><div className="rounded-xl bg-slate-50 px-3 py-2 sm:text-right"><p className="text-[9px] font-black uppercase tracking-wider text-slate-400">Pay</p><p className="mt-1 font-black tabular-nums text-slate-900">{pay}</p></div><button aria-label={`Remove ${employee.name}`} onClick={onRemove} className="grid h-10 w-10 place-items-center rounded-xl text-red-500 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button></div>;
}
