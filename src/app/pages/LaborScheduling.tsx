...
import { ArrowLeftRight, CalendarDays, Check, ChevronLeft, ChevronRight, Clock3, CloudSun, Copy, DollarSign, Eye, EyeOff, FileText, Filter, Plus, Save, Search, Settings2, SlidersHorizontal, Target, Trash2, UsersRound, X } from 'lucide-react';
...
function tagClass(tag = '') {
  ...
  if (tag.includes('OPEN')) return 'border-emerald-200 bg-emerald-50 text-[#F58220]';
  ...
  return 'border-lime-200 bg-lime-50 text-[#F58220]';
}
...
export function LaborScheduling() {
  ...
  const positionsInUse = useMemo(() => Array.from(new Set(employees.filter(employee => employee.active).map(employee => employee.role))).sort(), [employees]);
  const positions = useMemo(() => Array.from(new Set([...customPositions, ...positionsInUse])).sort(), [customPositions, positionsInUse]);
  const employeePositionOptions = useMemo(
    () => Array.from(new Set([...EMPLOYEE_POSITIONS, ...customPositions, ...employees.map(employee => employee.role), employeeRole].filter(Boolean)))
      .map(value => ({ value, label: value })),
    [customPositions, employees, employeeRole],
  );
  ...
  const addPosition = (event: FormEvent) => {
    ...
    const position = newPositionName.trim().replace(/\s+/g, ' ');
    if (!position) return toast.error('Enter a position name.');
    if ([...customPositions, ...positionsInUse].some(item => item.toLowerCase() === position.toLowerCase())) {
      return toast.error('That position already exists.');
    }
    updateSchedulerSettings({ customPositions: [...customPositions, position].sort((left, right) => left.localeCompare(right)) });
    setNewPositionName('');
    toast.success(`${position} added to position dropdowns.`);
  };

  const removePosition = (position: string) => {
    if (positionsInUse.some(item => item.toLowerCase() === position.toLowerCase())) {
      return toast.error('Reassign employees in this position before removing it.');
    }
    updateSchedulerSettings({ customPositions: customPositions.filter(item => item !== position) });
  ...
  const submitEmployee = async (event: FormEvent) => {
    ...
    try {
      ...
      const inviteResult = sendEmployeeInvite ? await inviteEmployee(employee) : null;
      if (!sendEmployeeInvite) addEmployee({ ...employee, inviteStatus: 'not-invited' });
      resetEmployeeForm();
      setInviteOpen(false);
      if (!sendEmployeeInvite) toast.success('Employee profile added. You can send app access later.');
      else if (inviteResult?.welcomeEmailSent) toast.success('Employee added. Welcome email and ZestEmployee link sent.');
  ...
  const submitShift = (event: FormEvent) => {
    ...
    const status = editingShiftId ? shifts.find(shift => shift.id === editingShiftId)?.status || 'scheduled' : 'scheduled';
    const shiftDetails = { employeeId, date: shiftDate, start: shiftStart, end: shiftEnd, breakMinutes: Number(shiftBreak) || 0, status, tag: shiftTag.trim().toUpperCase(), notes: shiftNotes.trim() };
    if (editingShiftId) updateShift(editingShiftId, shiftDetails);
    else addShift(shiftDetails);
    ...
    toast.success(editingShiftId ? 'Shift updated.' : 'Shift added to the schedule.');
  };

  const openEmployeeEditor = (employee: LaborEmployee) => navigate(`/app/labor/employees/${encodeURIComponent(employee.id)}`);
  ...
  const editShift = (shift: LaborShift) => {
    ...
    setShiftBreak(String(shift.breakMinutes || 0));
    setShiftTag(shift.tag || '');
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-5">
      <section className="overflow-hidden rounded-2xl bg-[#303A43] p-3 text-white sm:p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button type="button" onClick={() => navigate('/app/labor/employees/new')} className="inline-flex items-center gap-1.5 rounded-xl bg-[#F58220] px-3 py-2.5 text-xs font-black text-[#303A43]"><Plus className="h-3.5 w-3.5" />Add employee</button>
          <button type="button" onClick={() => setPositionSettingsOpen(true)} className="inline-flex items-center gap-1.5 rounded-xl border border-white/20 px-3 py-2.5 text-xs font-black text-white"><Settings2 className="h-3.5 w-3.5" />Positions</button>
          ...
          <section className="min-w-0 overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm">
            <div className="border-b border-slate-100 p-4">
              ...
              <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
                ...
                <button type="button" onClick={publishWeek} className={`ml-auto inline-flex h-10 shrink-0 items-center gap-2 rounded-xl px-4 text-xs font-black ${allVisiblePublished ? 'border border-slate-200 bg-white text-slate-700' : 'bg-[#303A43] text-white'}`}>{allVisiblePublished ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}{allVisiblePublished ? 'Unpublish' : 'Publish week'}</button>
              </div>
            ...
            <div className="overflow-x-auto">
              <table className={`w-full border-collapse text-left ${scheduleView === 'daily' ? 'min-w-[520px]' : 'min-w-[1180px]'}`}>
                ...
                <tbody>
                  ...
                  {showOpenShifts && <tr className="bg-lime-50/30"><th className="sticky left-0 z-10 border-b border-r border-slate-200 bg-lime-50 p-3 text-xs font-black text-slate-700">Open shifts</th>{visibleDays.map(day => { const key = localDateKey(day); const dayOpenShifts = openShifts.filter(item => item.date === key); return <td key={key} className="border-b border-r border-slate-200 p-2 align-top last:border-r-0"><div className="space-y-1">{dayOpenShifts.map(item => <button key={item.id} type="button" onClick={() => assignOpenShift(item.id)} className="block w-full rounded-lg border border-l-4 border-lime-200 border-l-orange-500 bg-white px-2 py-1.5 text-left text-[10px] text-slate-700" title="Click to assign"><span className="block font-black">{item.role}</span><span>{formatShiftTime(item.start, useAmPm)}–{formatShiftTime(item.end, useAmPm)}</span>{item.tag && <span className="mt-1 block text-[8px] font-black text-orange-700">{item.tag}</span>}</button>)}<button type="button" aria-label={`Add open shift on ${key}`} onClick={() => { setOpenShiftDate(key); setOpenShiftEditorOpen(true); }} className="grid h-7 w-full place-items-center rounded-lg border border-dashed border-lime-200 text-[#F58220]"><Plus className="h-3.5 w-3.5" /></button></div></td>; })}</tr>}
                  ...
                  const showPositionHeader = employeeIndex === 0 || activeEmployees[employeeIndex - 1]?.role !== employee.role;
                  return <Fragment key={employee.id}>{showPositionHeader && <tr className="bg-[#303A43]"><th colSpan={visibleDays.length + 1} className="px-4 py-2.5 text-xs font-black text-white"><div className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#F58220]" />{employee.role}</span><button type="button" onClick={() => { const publishKey = `${startKey}::${employee.role}`; updateSchedulerSettings({ publishedPositions: publishedPositions.includes(publishKey) ? publishedPositions.filter(key => key !== publishKey) : [...publishedPositions, publishKey] }); }} className="rounded-lg bg-white/10 px-2.5 py-1 text-[10px] font-black text-[#F58220]">{publishedPositions.includes(`${startKey}::${employee.role}`) ? 'Unpublish position' : 'Publish position'}</button></div></th></tr>}<tr className="align-top"><th className="sticky left-0 z-10 border-b border-r border-slate-200 bg-white p-3"><div className="flex items-start gap-2"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#FEF3C7] text-xs font-black text-[#303A43]">{employee.name.split(' ').map(part => part[0]).slice(0, 2).join('')}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-1.5"><p className="break-words text-sm font-black text-slate-900">{employee.name}</p>{publishedPositions.includes(`${startKey}::${employee.role}`) && <span className="rounded-full bg-emerald-50 px-1.5 py-0.5 text-[8px] font-black text-[#F58220]">PUBLISHED</span>}</div><p className="mt-0.5 break-words text-[10px] font-bold text-slate-500">{employee.role}</p><p className="mt-1 text-[10px] text-slate-400">{employeeWeekShifts.length} shifts · {employeeHours.toFixed(1)}h</p>{employee.payType === 'salary' && <span className="mt-1 inline-flex rounded-full bg-[#303A43] px-2 py-0.5 text-[9px] font-black text-[#F58220]">SALARIED</span>}</div></div></th>{visibleDays.map(day => {
                    const key = localDateKey(day);
      ...
      <section className="flex flex-col gap-3 rounded-3xl border border-lime-100 bg-lime-50 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-black text-[#F58220]">Target labour percentage</p><p className="mt-1 text-sm text-[#F58220]">Hourly scheduled labour plus prorated active salaries are measured against sales.</p></div><label className="flex items-center gap-2 font-black text-[#F58220]"><input aria-label="Target labour percentage" type="number" min="0" max="100" step="0.5" value={targetLaborPercent} onChange={event => setTargetLaborPercent(Number(event.target.value) || 0)} className="h-11 w-24 rounded-xl border border-lime-200 bg-white px-3 text-right" />%</label></section>

      <Dialog open={positionSettingsOpen} onOpenChange={setPositionSettingsOpen}><DialogContent className="max-h-[calc(100vh-2rem)] max-w-[calc(100vw-2rem)] overflow-y-auto sm:max-w-md"><DialogHeader><DialogTitle>Position settings</DialogTitle><DialogDescription>Add the positions used for employee profiles, open shifts and schedule views at this location.</DialogDescription></DialogHeader><form onSubmit={addPosition} className="flex gap-2"><label className="min-w-0 flex-1"><span className="sr-only">New position name</span><input value={newPositionName} onChange={event => setNewPositionName(event.target.value)} placeholder="e.g. Pastry Chef" maxLength={120} autoFocus className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm shadow-sm outline-none transition focus:border-[#D6B900] focus:ring-2 focus:ring-[#F58220]/30" /></label><button type="submit" className="inline-flex h-11 items-center gap-1.5 rounded-xl bg-[#F58220] px-4 text-sm font-black text-[#303A43]"><Plus className="h-4 w-4" />Add</button></form><div className="space-y-2"><p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Saved positions</p>{customPositions.length > 0 ? customPositions.map(position => { const inUse = positionsInUse.some(item => item.toLowerCase() === position.toLowerCase()); return <div key={position} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5"><div className="min-w-0"><p className="truncate text-sm font-bold text-slate-900">{position}</p>{inUse && <p className="mt-0.5 text-[10px] font-semibold text-slate-500">Assigned to an employee</p>}</div><button type="button" onClick={() => removePosition(position)} disabled={inUse} aria-label={`Remove ${position}`} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-red-500 hover:bg-red-50 disabled:cursor-not-allowed disabled:text-slate-300"><Trash2 className="h-4 w-4" /></button></div>; }) : <div className="rounded-xl bg-slate-50 px-4 py-5 text-center text-sm text-slate-500">No custom positions yet.</div>}</div>{positionsInUse.length > 0 && <div><p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Positions in use</p><div className="mt-2 flex flex-wrap gap-2">{positionsInUse.map(position => <span key={position} className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700">{position}</span>)}</div><p className="mt-2 text-xs leading-5 text-slate-500">Positions assigned to employees remain available and cannot be removed until those employees are reassigned.</p></div>}</DialogContent></Dialog>
      <Dialog open={inviteOpen} onOpenChange={setInviteOpen}><DialogContent className="max-h-[calc(100vh-2rem)] max-w-[calc(100vw-2rem)] overflow-y-auto sm:max-w-xl"><DialogHeader><DialogTitle>Add employee</DialogTitle><DialogDescription>Create an employee profile for schedules, pay and clock-in. App access is optional and can be sent now or later.</DialogDescription></DialogHeader><form onSubmit={submitEmployee} className="space-y-4"><div className="rounded-xl border border-lime-200 bg-lime-50 p-3"><label className="flex cursor-pointer items-start gap-3"><input type="checkbox" checked={sendEmployeeInvite} onChange={event => setSendEmployeeInvite(event.target.checked)} className="mt-1 h-4 w-4 accent-[#F58220]" /><span><span className="block text-sm font-black text-slate-900">Give ZestEmployee app access</span><span className="mt-0.5 block text-xs leading-5 text-slate-600">When selected, the employee receives a secure email to activate their account. Leave this off for a scheduling-only employee.</span></span></label></div><div className="grid gap-3 sm:grid-cols-2"><FormInput label="Full name" value={employeeName} onChange={setEmployeeName} placeholder="Employee name" required /><FormInput label={sendEmployeeInvite ? "Work email" : "Work email (optional)"} type="email" value={employeeEmail} onChange={setEmployeeEmail} placeholder="employee@restaurant.ca" required={sendEmployeeInvite} /><FormInput label="Phone" type="tel" value={employeePhone} onChange={setEmployeePhone} placeholder="416-555-0123" /><FormInput label="Clock-in number" value={employeeClockInNumber} onChange={setEmployeeClockInNumber} placeholder="1006" /><FormSelect label="Position" value={employeeRole} onChange={setEmployeeRole} options={employeePositionOptions} /><FormSelect label="Department" value={employeeDepartment} onChange={setEmployeeDepartment} options={['Management', 'Front of house', 'Back of house', 'Bar', 'Support'].map(value => ({ value, label: value }))} /><FormSelect label="Pay type" value={employeePayType} onChange={value => setEmployeePayType(value as 'hourly' | 'salary')} options={[{ value: 'hourly', label: 'Hourly' }, { value: 'salary', label: 'Salaried' }]} /></div>{employeePayType === 'salary' ? <FormInput label="Annual salary (CAD)" type="number" value={employeeSalary} onChange={setEmployeeSalary} /> : <FormInput label="Hourly rate (CAD)" type="number" value={employeeRate} onChange={setEmployeeRate} />}<div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => setInviteOpen(false)} className="rounded-xl border border-slate-200 px-4 py-2.5 font-bold text-slate-700">Cancel</button><button type="submit" disabled={isInviting} className="rounded-xl bg-[#F58220] px-4 py-2.5 font-black text-[#303A43] disabled:opacity-50">{isInviting ? 'Saving…' : sendEmployeeInvite ? 'Save & send app access' : 'Add employee'}</button></div></form></DialogContent></Dialog>
      <Dialog open={Boolean(editingEmployeeId)} onOpenChange={open => { if (!open) setEditingEmployeeId(null); }}><DialogContent className="max-h-[calc(100vh-2rem)] max-w-[calc(100vw-2rem)] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Employee profile</DialogTitle><DialogDescription>Manager-only personnel information. This is not shown in ZestEmployee.</DialogDescription></DialogHeader><form onSubmit={saveEmployeeChanges} className="space-y-5"><div className="flex items-center gap-3 rounded-2xl bg-[#303A43] p-4 text-white"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-[#F58220] text-lg font-black text-[#303A43]">{employeeName.split(' ').map(part => part[0]).slice(0, 2).join('') || 'EM'}</div><div><p className="font-black">{employeePreferredName || employeeName || 'Employee'}</p><p className="mt-1 text-sm text-slate-300">{employeeRole || 'Team member'} · {employeeDepartment || 'Restaurant team'}</p></div></div><section className="rounded-2xl border border-slate-200 p-4"><h3 className="font-black text-slate-900">Personal</h3><p className="mt-1 text-xs text-slate-500">Identity and contact details for managers.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><FormInput label="Full legal name" value={employeeName} onChange={setEmployeeName} required /><FormInput label="Preferred name" value={employeePreferredName} onChange={setEmployeePreferredName} placeholder="Name used on schedules" /><FormInput label="Date of birth" type="date" value={employeeBirthDate} onChange={setEmployeeBirthDate} /><FormInput label="Work email" type="email" value={employeeEmail} onChange={setEmployeeEmail} /></div></section><section className="rounded-2xl border border-slate-200 p-4"><h3 className="font-black text-slate-900">Contact & emergency contact</h3><div className="mt-4 grid gap-3 sm:grid-cols-2"><FormInput label="Primary phone" type="tel" value={employeePhone} onChange={setEmployeePhone} /><FormInput label="Alternate phone" type="tel" value={employeeAlternatePhone} onChange={setEmployeeAlternatePhone} /><FormInput label="Emergency contact name" value={employeeEmergencyContactName} onChange={setEmployeeEmergencyContactName} /><FormInput label="Emergency contact phone" type="tel" value={employeeEmergencyContactPhone} onChange={setEmployeeEmergencyContactPhone} /></div></section><section className="rounded-2xl border border-slate-200 p-4"><h3 className="font-black text-slate-900">Employment & access</h3><div className="mt-4 grid gap-3 sm:grid-cols-2"><FormInput label="Clock-in number" value={employeeClockInNumber} onChange={setEmployeeClockInNumber} /><FormSelect label="Position" value={employeeRole} onChange={setEmployeeRole} options={employeePositionOptions} /><FormSelect label="Department" value={employeeDepartment} onChange={setEmployeeDepartment} options={['Management', 'Front of house', 'Back of house', 'Bar', 'Support'].map(value => ({ value, label: value }))} /><FormSelect label="Pay type" value={employeePayType} onChange={value => setEmployeePayType(value as 'hourly' | 'salary')} options={[{ value: 'hourly', label: 'Hourly' }, { value: 'salary', label: 'Salaried' }]} /></div><div className="mt-3 rounded-xl bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-600">ZestEmployee access: <span className="font-black text-slate-900">{employees.find(employee => employee.id === editingEmployeeId)?.inviteStatus.replace('-', ' ') || 'not invited'}</span></div></section><section className="rounded-2xl border border-slate-200 p-4"><h3 className="font-black text-slate-900">Compensation</h3><div className="mt-4">{employeePayType === 'salary' ? <FormInput label="Annual salary (CAD)" type="number" value={employeeSalary} onChange={setEmployeeSalary} /> : <FormInput label="Hourly rate (CAD)" type="number" value={employeeRate} onChange={setEmployeeRate} />}</div></section><div className="flex justify-end gap-2 pt-1"><button type="button" onClick={() => setEditingEmployeeId(null)} className="rounded-xl border border-slate-200 px-4 py-2.5 font-bold text-slate-700">Cancel</button><button type="submit" className="rounded-xl bg-[#F58220] px-4 py-2.5 font-black text-[#303A43]">Save employee profile</button></div></form></DialogContent></Dialog>
      <Dialog open={shiftEditorOpen} onOpenChange={open => { setShiftEditorOpen(open); if (!open) { setOpenShiftBeingAssignedId(null); setEditingShiftId(null); } }}>
        <DialogContent className="max-h-[calc(100vh-2rem)] max-w-[calc(100vw-2rem)] overflow-y-auto sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{editingShiftId ? 'Edit shift' : openShiftBeingAssignedId ? 'Assign open shift' : 'Add a shift'}</DialogTitle>
            <DialogDescription>{editingShiftId ? 'Update the employee, date, times, break, tag or notes for this shift.' : 'Choose the time, tag and any handoff notes for this employee.'}</DialogDescription>
          </DialogHeader>
          <form onSubmit={submitShift} className="space-y-4">
            <FormSelect label="Employee" value={shiftEmployeeId || allActiveEmployees[0]?.id || ''} onChange={setShiftEmployeeId} options={allActiveEmployees.map(employee => ({ value: employee.id, label: `${employee.name} · ${employee.role}` }))} />
            <FormInput label="Date" type="date" value={shiftDate} onChange={setShiftDate} />
            <div className="grid grid-cols-2 gap-3"><FormInput label="Starts" type="time" value={shiftStart} onChange={setShiftStart} /><FormInput label="Ends" type="time" value={shiftEnd} onChange={setShiftEnd} /></div>
            <FormInput label="Unpaid break (min)" type="number" value={shiftBreak} onChange={setShiftBreak} />
            <FormInput label="Shift tag" value={shiftTag} onChange={setShiftTag} placeholder="CLOSE, EXPO, TRAINING…" list="shift-tags" />
            <datalist id="shift-tags">{SHIFT_TAGS.map(tag => <option key={tag} value={tag} />)}</datalist>
            <FormInput label="Shift notes" value={shiftNotes} onChange={setShiftNotes} placeholder="Section, training or handoff note" />
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" onClick={() => { setShiftEditorOpen(false); setOpenShiftBeingAssignedId(null); setEditingShiftId(null); }} className="rounded-xl border border-slate-200 px-4 py-3 font-bold text-slate-700">Cancel</button>
              <button type="submit" className="rounded-xl bg-[#F58220] px-4 py-3 font-black text-[#303A43]">{editingShiftId ? 'Save changes' : openShiftBeingAssignedId ? 'Assign shift' : 'Add tagged shift'}</button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
      <Dialog open={openShiftEditorOpen} onOpenChange={setOpenShiftEditorOpen}><DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-md"><DialogHeader><DialogTitle>Add an open shift</DialogTitle><DialogDescription>Create an unassigned house shift that a manager can fill later.</DialogDescription></DialogHeader><form onSubmit={submitOpenShift} className="space-y-4"><FormSelect label="Position" value={openShiftRole} onChange={setOpenShiftRole} options={(positions.length > 0 ? positions : ['Support']).map(position => ({ value: position, label: position }))} /><FormInput label="Date" type="date" value={openShiftDate} onChange={setOpenShiftDate} /><div className="grid grid-cols-2 gap-3"><FormInput label="Starts" type="time" value={openShiftStart} onChange={setOpenShiftStart} /><FormInput label="Ends" type="time" value={openShiftEnd} onChange={setOpenShiftEnd} /></div><FormInput label="Shift tag" value={openShiftTag} onChange={setOpenShiftTag} list="shift-tags" /><div className="flex justify-end gap-2"><button type="button" onClick={() => setOpenShiftEditorOpen(false)} className="rounded-xl border border-slate-200 px-4 py-2.5 font-bold">Cancel</button><button type="submit" className="rounded-xl bg-[#F58220] px-4 py-2.5 font-black text-[#303A43]">Add open shift</button></div></form></DialogContent></Dialog>
    </div>
  );
}
...
function ToolbarButton({ icon: Icon, label, active = false, onClick }: { icon: typeof CalendarDays; label: string; active?: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`inline-flex h-10 shrink-0 items-center gap-2 rounded-xl border px-3 text-xs font-black ${active ? 'border-[#F58220] bg-lime-50 text-[#7A5D00]' : 'border-slate-200 bg-white text-slate-700'}`}><Icon className="h-4 w-4" />{label}</button>;
}

function OptionToggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return <label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700"><span>{label}</span><input type="checkbox" checked={checked} onChange={event => onChange(event.target.checked)} className="h-4 w-4 accent-[#F58220]" /></label>;
}

function ViewButton({ label, active, count = 0, onClick }: { label: string; active: boolean; count?: number; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`relative rounded-lg px-3 py-2 text-sm font-bold ${active ? 'bg-[#F58220] text-[#303A43]' : 'text-white/65'}`}>{label}{count > 0 && <span className="ml-1 rounded-full bg-red-500 px-1.5 py-0.5 text-[9px] text-white">{count}</span>}</button>;
}
...
function FormInput({ label, value, onChange, type = 'text', placeholder, list, required = false }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string; list?: string; required?: boolean }) {
  return <label className="block"><span className="mb-1.5 block text-[10px] font-black uppercase tracking-wider text-slate-500">{label}</span><input type={type} value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} list={list} required={required} min={type === 'number' ? 0 : undefined} step={type === 'number' ? '0.01' : undefined} className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm shadow-sm outline-none transition focus:border-[#D6B900] focus:ring-2 focus:ring-[#F58220]/30" /></label>;
}
...
function RequestQueue({ title, empty, icon: Icon, children }: { title: string; empty: string; icon: typeof CalendarDays; children: ReactNode }) {
  ...
  return <section className="overflow-hidden rounded-3xl bg-white shadow-sm"><div className="flex items-center gap-2 border-b border-slate-100 p-5"><Icon className="h-5 w-5 text-[#F58220]" /><h2 className="font-black">{title}</h2></div><div className="divide-y divide-slate-100">{hasRequests ? children : <p className="p-8 text-center text-sm text-slate-500">{empty}</p>}</div></section>;
}
...
function ManagerRequest({ title, detail, status, onApprove, onDecline }: { title: string; detail: string; status: string; onApprove: () => void; onDecline: () => void }) {
  return <div className="p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="break-words font-black text-slate-900">{title}</p><p className="mt-1 break-words text-sm leading-6 text-slate-500">{detail}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${status === 'pending' ? 'bg-lime-50 text-[#F58220]' : status === 'approved' ? 'bg-emerald-50 text-[#F58220]' : 'bg-slate-100 text-slate-500'}`}>{status}</span></div>{status === 'pending' && <div className="mt-4 flex gap-2"><button onClick={onApprove} className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-[#F58220] py-2.5 text-sm font-black text-white"><Check className="h-4 w-4" />Approve</button><button onClick={onDecline} className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-red-200 py-2.5 text-sm font-black text-red-600"><X className="h-4 w-4" />Decline</button></div>}</div>;
}
