    ...
    <div className="mx-auto max-w-[1500px] space-y-5">
      <section className="overflow-hidden rounded-2xl bg-[#303A43] p-3 text-white sm:p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button type="button" onClick={() => navigate('/app/labor/employees/new')} className="inline-flex items-center gap-1.5 rounded-xl bg-[#F5D62E] px-3 py-2.5 text-xs font-black text-[#303A43]"><Plus className="h-3.5 w-3.5" />Add employee</button>
...
function ToolbarButton({ icon: Icon, label, active = false, onClick }: { icon: typeof CalendarDays; label: string; active?: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`inline-flex h-10 shrink-0 items-center gap-2 rounded-xl border px-3 text-xs font-black ${active ? 'border-[#F5D62E] bg-amber-50 text-[#7A5D00]' : 'border-slate-200 bg-white text-slate-700'}`}><Icon className="h-4 w-4" />{label}</button>;
}

function OptionToggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return <label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700"><span>{label}</span><input type="checkbox" checked={checked} onChange={event => onChange(event.target.checked)} className="h-4 w-4 accent-[#F5D62E]" /></label>;
}

function ViewButton({ label, active, count = 0, onClick }: { label: string; active: boolean; count?: number; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`relative rounded-lg px-3 py-2 text-sm font-bold ${active ? 'bg-[#F5D62E] text-[#303A43]' : 'text-white/65'}`}>{label}{count > 0 && <span className="ml-1 rounded-full bg-red-500 px-1.5 py-0.5 text-[9px] text-white">{count}</span>}</button>;
}
