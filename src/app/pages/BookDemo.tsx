...
const fieldClass = 'mt-1 h-12 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-900 outline-none transition focus:border-[#F58220] focus:ring-4 focus:ring-[#F58220]/15';
...
export function BookDemo() {
  ...
  return (
    <main className="grey-green-preview min-h-screen bg-[#F1EFEB] px-4 py-4 text-[#303A43] sm:px-7 sm:py-7">
      <div className="mx-auto grid max-w-[1440px] gap-3 lg:min-h-[calc(100vh-3.5rem)] lg:grid-cols-[0.98fr_1.02fr]">
        <section className="rounded-[28px] bg-white p-6 shadow-sm sm:p-10 lg:p-12">
          ...
          <Link to="/" aria-label="ZestIQ home" className="mt-10 inline-flex rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F58220]"><ZestIQBrand className="gap-3" markClassName="h-14 w-14 rounded-none bg-transparent" wordmarkClassName="text-3xl" /></Link>
        ...
        <section className="flex flex-col gap-3">
          <div className="relative flex min-h-[520px] flex-1 overflow-hidden rounded-[28px] bg-[#F58220] p-7 sm:p-10 lg:p-12">
            <div className="relative z-10 flex w-full flex-col">
              ...
              <div className="mt-8 rounded-[24px] bg-[#303A43] p-4 text-white shadow-2xl sm:ml-auto sm:w-[82%]">
                <div className="flex items-center justify-between border-b border-white/10 pb-3"><div><p className="text-[9px] font-black uppercase tracking-wider text-white/40">Today’s margin brief</p><p className="mt-1 font-black">3 actions worth reviewing</p></div><span className="rounded-full bg-[#F58220]/15 px-2 py-1 text-[9px] font-black text-[#F58220]">LIVE</span></div>
                <div className="mt-3 grid grid-cols-3 gap-2">{[['Food cost', '28.4%'], ['Labour', '29.7%'], ['At risk', '$624']].map(([label, value]) => <div key={label} className="rounded-xl bg-white/8 p-3"><p className="text-[8px] font-black uppercase text-white/35">{label}</p><p className="mt-1 text-lg font-black text-[#F58220]">{value}</p></div>)}</div>
                <div className="mt-3 space-y-2">{['Olive oil cost increased 15%', 'Friday labour is above target', '12 items are below par'].map((item, index) => <div key={item} className="flex items-center justify-between rounded-xl bg-white/8 px-3 py-2.5 text-xs font-bold"><span>{item}</span><span className="grid h-6 w-6 place-items-center rounded-full bg-[#F58220] text-[#303A43]">{index + 1}</span></div>)}</div>
              </div>
          ...
          <div className="flex items-center justify-center gap-2 rounded-[22px] bg-[#303A43] p-4 text-center text-xs font-bold text-white/65"><ShieldCheck className="h-4 w-4 text-[#F58220]" />No free-trial pressure. A focused product walkthrough for your operation.</div>
        </section>
      </div>
    </main>
  );
...
export function BookDemoThankYou() {
  ...
  return (
    <main className="grey-green-preview grid min-h-screen place-items-center bg-[#F1EFEB] px-4 py-10 text-[#303A43]">
      <section className="w-full max-w-2xl rounded-[30px] bg-white p-8 text-center shadow-xl sm:p-12">
        <Link to="/" aria-label="ZestIQ home" className="inline-flex rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F58220]"><ZestIQBrand className="gap-3" markClassName="h-14 w-14 rounded-none bg-transparent" wordmarkClassName="text-3xl" /></Link>
        <div className="mx-auto mt-8 grid h-16 w-16 place-items-center rounded-2xl bg-[#F58220]"><CheckCircle2 className="h-8 w-8" /></div>
        <p className="mt-7 text-xs font-black uppercase tracking-[0.2em] text-[#F58220]">Inquiry received</p>
