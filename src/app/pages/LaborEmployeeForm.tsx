...
export function LaborEmployeeForm() {
  ...
  return <div className="mx-auto max-w-5xl space-y-5 pb-10">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center gap-3"><button type="button" onClick={() => navigate('/app/labor')} className="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-white text-slate-700 shadow-sm" aria-label="Back to labour"><ArrowLeft className="h-5 w-5" /></button><div><p className="text-xs font-black uppercase tracking-[0.16em] text-[#F58220]">Labour · Employees</p><h1 className="mt-1 text-2xl font-black text-slate-950 sm:text-3xl">{isNew ? 'Add employee' : 'Employee profile'}</h1><p className="mt-1 text-sm text-slate-500">{isNew ? 'Create the complete employee record before adding them to the schedule.' : 'Update personal, employment and compensation information.'}</p></div></div>
      <button type="submit" form="employee-form" disabled={isSaving} className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#F58220] px-5 text-sm font-black text-[#303A43] disabled:opacity-50"><Save className="h-4 w-4" />{isSaving ? 'Saving…' : isNew ? 'Save employee' : 'Save changes'}</button>
    </div>

    <form id="employee-form" onSubmit={submit} className="grid gap-5">
      <div className="space-y-5">
        {isNew && <section className="rounded-2xl border border-lime-200 bg-lime-50 p-4"><label className="flex cursor-pointer items-start gap-3"><input type="checkbox" checked={sendEmployeeInvite} onChange={event => setSendEmployeeInvite(event.target.checked)} className="mt-1 h-4 w-4 accent-[#F58220]" /><span><span className="block text-sm font-black text-slate-900">Give ZestEmployee app access</span><span className="mt-1 block text-xs leading-5 text-slate-600">Send a secure activation email after this profile is saved. Turn this off for a scheduling-only employee.</span></span></label></section>}
        ...
        <FormSection title="POS time clock" description="Match this employee to the same person in your POS to bring clocked hours into ZestIQ.">
          {!isPosConnected ? <div className="flex flex-col gap-3 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-slate-500 shadow-sm"><Unplug className="h-4 w-4" /></span><div><p className="text-sm font-black text-slate-900">No POS connected</p><p className="mt-1 text-xs leading-5 text-slate-500">Connect a POS for this location before matching employee time-clock records.</p></div></div><button type="button" onClick={() => navigate('/app/integrations')} className="h-10 rounded-xl bg-[#303A43] px-4 text-xs font-black text-white">Open integrations</button></div> : <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-[#303A43] p-4 text-white"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#F58220] font-black text-[#303A43]">{posProvider.mark}</span><div><p className="text-sm font-black">{posProvider.name}</p><p className="mt-0.5 text-xs text-slate-300">{connectionMode === 'direct' ? 'Direct connection' : 'File import connection'}</p></div></div><span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-[#F58220]"><CheckCircle2 className="h-3.5 w-3.5" />Connected</span></div>
            <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto]"><Field label="POS employee ID" value={posEmployeeId} onChange={value => { setPosEmployeeId(value); if (value.trim() !== employee?.posEmployeeId) setPosMappingVerifiedAt(''); }} placeholder={`ID from ${posProvider.name}`} /><button type="button" onClick={verifyPosMapping} className="mt-[18px] inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#F58220] px-4 text-xs font-black text-[#303A43]"><Link2 className="h-4 w-4" />{posMappingVerifiedAt ? 'Recheck match' : 'Link employee'}</button></div>
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => navigate('/app/labor')} className="h-11 rounded-xl border border-slate-200 bg-white px-5 text-sm font-bold text-slate-700">Cancel</button><button type="submit" disabled={isSaving} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#F58220] px-5 text-sm font-black text-[#303A43] disabled:opacity-50"><Save className="h-4 w-4" />{isSaving ? 'Saving…' : isNew ? 'Save employee' : 'Save changes'}</button></div>
    </form>
  </div>;
}
