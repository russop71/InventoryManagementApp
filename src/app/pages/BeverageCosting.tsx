...
const BEVERAGE_CATEGORIES = ['White Wine', 'Red Wine', 'Rosé Wine', 'Sparkling Wine', 'Wine', 'Liquor', 'Beer', 'Beverage'];
...
export function BeverageCosting() {
  const { inventory, recipes, updateRecipe, updateInventoryItem } = useInventory();
  const { user } = useAuth();
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
      <section className="overflow-hidden rounded-[30px] bg-[#303A43] p-6 text-white sm:p-8"><div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-[#F58220]">Full restaurant mode</p><h1 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">Liquor, wine & beer costing.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-white/60">Track bottle and case inventory, calculate yields and pour cost, and see which drinks are protecting—or draining—beverage margin.</p></div><Link to="/app/inventory" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#F58220] px-4 py-3 font-black text-[#303A43]"><PackagePlus className="h-4 w-4" />Manage bar inventory</Link></div></section>
          ...
          {editingRecipe && (
            <div className="space-y-5">
              ...
              <div className="flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:justify-end">
                ...
                <button type="button" onClick={saveRecipeEditor} className="rounded-xl bg-[#F58220] px-5 py-2.5 font-black text-[#303A43]">Save recipe costing</button>
              </div>
            </div>
          )}
        </DialogContent>
