...
export function BeverageCosting() {
  ...
  const saveRecipeEditor = () => {
    ...
    updateRecipe(editingRecipe.id, { menuItemName: editingRecipeName.trim(), price });
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
      ...
      <Dialog open={Boolean(editingRecipeId)} onOpenChange={open => !open && setEditingRecipeId(null)}><DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-lg"><DialogHeader><DialogTitle>Edit beverage menu item</DialogTitle><DialogDescription>Update the menu name and selling price. Ingredient costs continue to come from the linked inventory items.</DialogDescription></DialogHeader>{editingRecipe && <div className="space-y-4"><label className="block"><span className="mb-1 block text-xs font-black uppercase tracking-wider text-slate-500">Menu item name</span><input value={editingRecipeName} onChange={event => setEditingRecipeName(event.target.value)} className="h-11 w-full rounded-xl border border-slate-200 px-3 font-bold" /></label><label className="block"><span className="mb-1 block text-xs font-black uppercase tracking-wider text-slate-500">Selling price (CAD)</span><input type="number" min="0" step="0.01" value={editingRecipePrice} onChange={event => setEditingRecipePrice(event.target.value)} className="h-11 w-full rounded-xl border border-slate-200 px-3 font-bold" /></label><div className="rounded-2xl bg-slate-50 p-3"><p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Recipe components</p><div className="mt-2 space-y-1 text-sm text-slate-700">{editingRecipe.ingredients.map(ingredient => <p key={`${ingredient.inventoryItemId}-${ingredient.quantity}`}>{inventory.find(item => item.id === ingredient.inventoryItemId)?.name || ingredient.inventoryItemId} · {ingredient.quantity} {ingredient.unit}</p>)}</div></div><div className="flex justify-end gap-2"><button type="button" onClick={() => setEditingRecipeId(null)} className="rounded-xl border border-slate-200 px-4 py-2.5 font-bold text-slate-700">Cancel</button><button type="button" onClick={saveRecipeEditor} className="rounded-xl bg-[#F5C10E] px-4 py-2.5 font-black text-[#0B1220]">Save changes</button></div></div>}</DialogContent></Dialog>
    </div>
  );
}
