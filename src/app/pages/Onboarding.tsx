  ...
  return (
    <aside className="rounded-[28px] bg-[#303A43] p-5 text-white lg:sticky lg:top-28 lg:self-start">
      <div className="flex min-w-0 items-center gap-3 border-b border-white/10 pb-5">
        <div className="grid h-11 w-11 place-items-center rounded-2xl bg-[#F58220] text-[#303A43]"><Rocket className="h-5 w-5" /></div>
        ...
        {STEPS.map((step, index) => {
          ...
          return (
            <li key={step.id} aria-current={active ? 'step' : undefined} className={`flex min-w-0 items-center gap-2.5 rounded-2xl p-2.5 ${active ? 'bg-white text-[#303A43]' : 'text-white/60'}`}>
              <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-xl ${complete ? 'bg-[#F58220] text-white' : active ? 'bg-[#F58220] text-[#303A43]' : 'bg-white/10'}`}>{complete ? <Check className="h-4 w-4" /> : <Icon className="h-4 w-4" />}</span>
      ...
      <div className="mt-5 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-[#F58220]" style={{ width: `${Math.max(8, ((currentIndex + 1) / STEPS.length) * 100)}%` }} /></div>
...
function StepHeading({ step }: { step: (typeof STEPS)[number] }) {
  return <div><p className="text-xs font-black uppercase tracking-[0.2em] text-[#F58220]">Step {STEPS.findIndex(item => item.id === step.id) + 1} of {STEPS.length}</p><h1 className="mt-2 text-3xl font-black tracking-[-0.03em] text-[#303A43] sm:text-4xl">{step.label}</h1><p className="mt-3 max-w-2xl text-base leading-7 text-slate-600">{step.description}</p></div>;
}
  ...
  if (!canManage) {
    return (
      <div className="mx-auto max-w-xl rounded-3xl bg-white p-8 text-center shadow-sm">
        <Building2 className="mx-auto h-12 w-12 text-[#F58220]" />
  ...
  return (
    <div className="mx-auto max-w-6xl py-3 sm:py-6">
      <div className="mb-5 flex items-center justify-between gap-4">
        <div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#F58220]">Launch your workspace</p><p className="mt-1 text-sm text-slate-500">Connect the restaurant data that powers ZestIQ.</p></div>
          ...
          <div className="mt-8">
            {currentStep.id === 'restaurant' && (
              <form onSubmit={saveRestaurant} className="space-y-5">
                <div><FieldLabel htmlFor="restaurant-name">Restaurant or company name</FieldLabel><input id="restaurant-name" value={restaurantName} onChange={event => setRestaurantName(event.target.value)} className="h-13 w-full rounded-2xl border border-slate-200 px-4 text-base font-semibold outline-none focus:border-[#F58220]" placeholder="e.g. North & Vine" /></div>
                <div className="rounded-2xl bg-lime-50 p-4 text-sm leading-6 text-[#F58220]"><Sparkles className="mr-2 inline h-4 w-4" />This name stays inside your company workspace and appears on reports.</div>
                <StepActions saving={saving} label="Continue" />
              </form>
            )}
            {currentStep.id === 'location' && (
              <form onSubmit={saveLocation} className="space-y-5">
                <div><FieldLabel htmlFor="location-name">Location name</FieldLabel><input id="location-name" value={locationName} onChange={event => setLocationName(event.target.value)} className="h-13 w-full rounded-2xl border border-slate-200 px-4 font-semibold outline-none focus:border-[#F58220]" placeholder="e.g. King Street" /></div>
...
function StepActions({ saving, label }: { saving: boolean; label: string }) {
  return (
    <div className="flex justify-end border-t border-slate-100 pt-5">
      <button type="submit" disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#F58220] px-5 py-3 font-black text-[#303A43] disabled:opacity-50">{saving ? 'Saving…' : label}<ArrowRight className="h-4 w-4" /></button>
    </div>
  );
}
