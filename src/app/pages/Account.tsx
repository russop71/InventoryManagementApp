...
import { AccountLocations } from '../components/AccountLocations';

export function Account() {
  const { user, accountId, accountName, locations, logout, changePassword, updateAccountProfile, updateLocalAccountProfile } = useAuth();
  ...
  if (isDemoAccount) {
    return (
      <div className="space-y-4">
        ...
        <Card className="border-lime-200 bg-lime-50"><CardContent className="py-5"><KeyRound className="mb-3 h-8 w-8 text-[#F58220]" /><p className="font-semibold text-[#F58220]">Account administration is disabled in the public demo.</p><p className="mt-1 text-sm text-[#F58220]">Profile editing, passwords, two-factor authentication, locations and data reset are available only in a private workspace.</p></CardContent></Card>
