...
export function Mfa() {
  ...
  useEffect(() => {
    ...
    const loadStatus = async () => {
      setStatusError(null);
      try {
        const result = await apiRequest<MfaStatus>('/api/v1/auth/mfa/status');
        if (active) setStatus(result);
      } catch (error) {
        if (!active) return;
        const message = error instanceof Error ? error.message : 'We could not check account security right now.';
  ...
  return <main className="grid min-h-screen place-items-center bg-[#F58220] p-5"><section className="w-full max-w-md rounded-[30px] bg-white p-7 shadow-2xl sm:p-9"><ZestIQBrand markClassName="h-12 w-12 rounded-xl" wordmarkClassName="text-2xl" /><div className="mt-8 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#303A43] text-[#F58220]"><ShieldCheck /></div><h1 className="mt-4 text-3xl font-black tracking-tight text-[#303A43]">Protect your account</h1><p className="mt-2 text-sm leading-6 text-slate-600">{setupOptional ? 'Set up two-step verification now to protect your ZestIQ access. It is optional during the current rollout.' : 'Two-step verification is required for this account.'}</p>{!status && !statusError ? <p className="mt-7 text-sm text-slate-500">Checking account security…</p> : statusError ? <div className="mt-7 rounded-2xl border border-lime-200 bg-lime-50 p-4 text-sm text-[#F58220]"><p>We could not load two-step verification.</p><p className="mt-1 text-[#F58220]">You can safely continue for now while we restore the connection.</p></div> : !canSetUp ? <p className="mt-7 rounded-xl bg-slate-50 p-4 text-sm text-slate-600">Two-step verification can be set up by company owners, administrators, and ZestIQ platform administrators.</p> : !enrollment && !hasExistingFactor ? <button type="button" disabled={busy} onClick={startEnrollment} className="mt-7 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#303A43] font-black text-white disabled:opacity-50"><Smartphone className="h-4 w-4 text-[#F58220]" />Set up authenticator app</button> : <form onSubmit={verify} className="mt-7 space-y-4">{enrollment?.qrCode && <><p className="text-sm font-bold text-[#303A43]">Scan this QR code with Google Authenticator, Microsoft Authenticator, 1Password, or another authenticator app.</p><div className="flex justify-center rounded-2xl border border-slate-200 bg-white p-4">{qrFailed ? <div className="flex h-48 w-48 flex-col items-center justify-center text-center text-sm text-slate-600"><p>The QR image could not load.</p><button type="button" className="mt-3 rounded-lg border px-3 py-2 font-bold" onClick={() => void copyManualSetupLink()}>Copy manual setup link</button></div> : <img src={enrollment.qrCode} alt="ZestIQ authenticator QR code" onError={() => setQrFailed(true)} className="h-48 w-48" />}</div><button type="button" onClick={() => void copyManualSetupLink()} className="w-full text-sm font-bold text-slate-600 underline">Can’t scan? Copy a manual setup link</button></>} {hasExistingFactor && !enrollment && <p className="rounded-xl bg-slate-50 p-3 text-sm text-slate-600">An authenticator is already enrolled. Enter its current code below.</p>}<label className="block text-sm font-black text-slate-700">Six-digit code<input autoFocus inputMode="numeric" maxLength={6} value={code} onChange={event => setCode(event.target.value.replace(/\D/g, ''))} className="mt-2 h-12 w-full rounded-xl border border-slate-300 px-4 text-center text-xl font-black tracking-[.35em] outline-none focus:border-[#F58220]" /></label><button disabled={busy || code.length !== 6} className="h-12 w-full rounded-xl bg-[#303A43] font-black text-white disabled:opacity-50">Verify and continue</button></form>}{setupOptional && <button type="button" disabled={busy} onClick={skipForNow} className="mt-4 w-full text-sm font-black text-slate-600 underline disabled:opacity-50">Skip for now</button>}<button type="button" onClick={logout} className="mt-5 w-full text-sm font-bold text-slate-500 underline">Sign out</button></section></main>;
}
