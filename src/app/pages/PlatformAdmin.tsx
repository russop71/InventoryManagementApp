...
interface ClientDetail {
  ...
  users: Array<{ id: string; name: string; email: string; role: string; status: string; lastLogin: string; locationIds: string[] }>;
...
function healthFor(client: ClientSummary) {
  ...
  const tone = score >= 75 ? 'bg-emerald-100 text-[#F58220]' : score >= 45 ? 'bg-lime-100 text-[#F58220]' : 'bg-red-100 text-red-800';
...
export function PlatformAdmin() {
  ...
  const executive = useMemo(() => {
    ...
    const actionItems = [
      ...
      ...clients.filter(client => client.billing.approvalStatus === 'pending_ceo_approval').map(client => ({ client, title: 'CEO approval ready', detail: `${client.name} completed payment and is ready for final account approval.`, tone: 'text-[#303A43] bg-[#FFF7C2] border-[#F58220]' })),
      ...clients.filter(client => client.billing.status === 'not_configured').map(client => ({ client, title: 'Billing is not configured', detail: `${client.name} has no active subscription. Create a checkout link when the client is ready.`, tone: 'text-[#F58220] bg-lime-50 border-lime-200' })),
      ...
      ...health.filter(item => item.inactiveDays >= 14 && Number.isFinite(item.inactiveDays)).map(item => ({ client: item.client, title: 'Client may need outreach', detail: `${item.client.name} has not been active for ${item.inactiveDays} days.`, tone: 'text-[#F58220] bg-lime-50 border-lime-200' })),
    ].slice(0, 6);
  ...
  const saveClientUser = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await apiRequest(`/api/v1/platform/accounts/${encodeURIComponent(selectedClient.id)}/users/${encodeURIComponent(selectedClientUser.id)}`, {
        method: 'PUT',
        body: JSON.stringify({ name: String(formData.get('name') || '').trim(), email: String(formData.get('email') || '').trim(), role: String(formData.get('role') || ''), status: String(formData.get('status') || ''), locationIds: formData.getAll('locationIds').map(String) }),
      });
    ...
    return (
      <Card className="mx-auto max-w-lg border-[#E7D88A] shadow-sm">
        <CardContent className="p-6 sm:p-8">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#303A43] text-[#F58220]"><LockKeyhole className="h-6 w-6" /></div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#F58220]">ZestIQ platform administration</p>
      ...
      <section className="overflow-hidden rounded-3xl bg-[#303A43] p-5 text-white shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#F58220]">CEO action centre</p>
          ...
          <div className="rounded-2xl bg-white/10 px-4 py-3"><p className="text-xs uppercase tracking-wide text-slate-300">Accounts at risk</p><p className="mt-1 text-2xl font-extrabold text-[#F58220]">{executive.atRisk}</p></div>
        </div>
        <div className="mt-4 grid gap-3 lg:grid-cols-2">
          {executive.actionItems.length ? executive.actionItems.map(item => (
            <button key={`${item.title}-${item.client.id}`} type="button" onClick={() => void openClient(item.client.id)} className="flex items-start gap-3 rounded-2xl bg-white/10 p-4 text-left transition hover:bg-white/15">
              <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-[#F58220]" />
          ...
          )) : <div className="flex items-center gap-3 rounded-2xl bg-[#F58220]/15 p-4 text-emerald-100"><CheckCircle2 className="h-5 w-5" />No urgent billing or client-health issues right now.</div>}
        </div>
      </section>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle className="text-base">Security & platform safeguards</CardTitle></CardHeader><CardContent className="space-y-3 text-sm text-slate-600">
          <p className="flex gap-2"><ShieldCheck className="h-4 w-4 shrink-0 text-[#F58220]" /><span>Client workspaces are isolated by company account.</span></p>
          <p className="flex gap-2"><CreditCard className="h-4 w-4 shrink-0 text-[#F58220]" /><span>Card details stay in Stripe; ZestIQ only displays safe payment metadata.</span></p>
          <p className="flex gap-2"><UserCheck className="h-4 w-4 shrink-0 text-[#F58220]" /><span>CEO access is separate from restaurant manager access.</span></p>
        </CardContent></Card>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {isLoading && clients.length === 0 ? (
            ...
            <button key={client.id} type="button" onClick={() => void openClient(client.id)} className="w-full rounded-2xl border border-slate-200 p-4 text-left transition hover:border-[#F58220] hover:bg-[#FEFCE8]/40">
              <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          {selectedClient && (
            <>
              ...
              {selectedClient.billing.approvalStatus === 'pending_ceo_approval' && (
                <div className="rounded-2xl border-2 border-[#F58220] bg-[#FFFCED] p-4">
              ...
              {selectedClient.billing.approvalStatus === 'approved' && selectedClient.onboarding?.clientProfile?.approvalRequired === true && (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-[#F58220]"><strong>CEO approved</strong>{selectedClient.onboarding.clientProfile.ceoApprovedAt ? ` · ${formatDate(selectedClient.onboarding.clientProfile.ceoApprovedAt)}` : ''}</div>
              )}

              <div className="rounded-2xl border border-[#F58220] bg-[#FFFCED] p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <Badge className={selectedClient.onboarding?.clientProfile?.schedulingEnabled !== false ? 'bg-emerald-100 text-[#F58220]' : 'bg-slate-200 text-slate-700'}>{selectedClient.onboarding?.clientProfile?.schedulingEnabled !== false ? 'Enabled' : 'Not included'}</Badge>
                </div>
              ...
              <div className="rounded-2xl border border-slate-200 p-4">
                ...
                <div className="mt-3 rounded-xl border border-lime-200 bg-lime-50 p-3 text-sm text-slate-700">
                ...
                {!selectedClient.billing.configured && <p className="mt-3 text-xs text-[#F58220]">Connect Stripe keys and price IDs before creating payment links.</p>}
                {selectedClient.onboarding?.clientProfile?.schedulingEnabled !== false && !selectedClient.billing.schedulingPriceConfigured && <p className="mt-3 text-xs text-[#F58220]">Add the CAD $49.99 monthly Scheduling price ID in Stripe/Vercel before creating this checkout link.</p>}
                {billableLocationCount > 1 && selectedClient.onboarding?.clientProfile?.schedulingEnabled !== false && !selectedClient.billing.additionalLocationSchedulingPriceConfigured && <p className="mt-3 text-xs text-[#F58220]">Add the CAD $24.99 additional-location Scheduling price ID in Stripe/Vercel before creating this checkout link.</p>}
                {paymentLink && (
                  <div className="mt-3 flex flex-wrap gap-2">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 p-4">
                  ...
                  <div className="mt-3 space-y-2">{selectedClient.users.map(member => <button key={member.id} type="button" onClick={() => setSelectedClientUser(member)} className="w-full rounded-xl bg-slate-50 p-3 text-left transition hover:bg-lime-50"><p className="text-sm font-medium">{member.name} · {member.role}</p><p className="text-xs text-slate-500">{member.email} · {member.status}</p></button>)}</div>
